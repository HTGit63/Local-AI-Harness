/* eslint-disable react-refresh/only-export-components -- AgentMode.tsx is the refresh boundary; this hook owns legacy Agent Mode JSX during decomposition. */
...
import { API, fetchJson } from '../agentApi';
import { AgentRunConsole } from '../AgentRunConsole';
import { AgentRunLogPanel } from '../AgentRunLogPanel';
...
const AGENT_USER_MODES = USER_MODES.filter((mode) => mode.id !== 'chat');
...
function modeFromPolicyMode(mode: string | undefined): UserMode {
  if (mode === 'chat') return 'full-agent';
...
export function useAgentModeController({ onBack, onOpenChat }: AgentModeProps) {
  // UI state
  ...
  const [userMode, setUserMode] = useState<UserMode>(() => {
    if (typeof window === 'undefined') return 'full-agent';
    const stored = window.localStorage.getItem(MODE_STORAGE_KEY);
    if (isUserMode(stored)) return stored === 'chat' ? 'full-agent' : stored;
    if (stored === 'agent') return 'full-agent';
    if (stored === 'chat') return 'full-agent';
    const legacy = window.localStorage.getItem(LEGACY_AGENTIC_STORAGE_KEY);
    return legacy === 'true' ? 'full-agent' : 'full-agent';
  });
  ...
  return (
    <div className="app-shell">
      <AgentTopBar
        ...
        runtimeFallbackWarning={runtimeFallbackWarning}
        ...
        onNewThread={() => { void startNewSession(); }}
        onRefresh={() => { void refreshDashboard('full', { includeHeavy: true }); }}
              ...
              <div className="composer-actions">
                <div className="composer-actions-left">
                  <select
                    ...
                    title={userModeNote}
                  >
                    {AGENT_USER_MODES.map(m => <option key={m.id} value={m.id}>{m.label}</option>)}
                  </select>
                  {executionMode === 'agent' && (
...
export default useAgentModeController;
